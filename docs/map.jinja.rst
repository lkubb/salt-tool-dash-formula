...
We will see a quick setup to configure the ``tool_dash`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_dash`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_dash`` formulas under ``/srv/salt/tool_dash/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_dash/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_dash/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_dash/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_dash/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_dash`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_dash`` formulas under ``/srv/salt/tool_dash/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_dash/parameters/roles
...
We create a configuration for the role ``tool_dash/server`` in ``/srv/salt/tool_dash/parameters/roles/tool_dash/server.yaml``:
...
We create another configuration for the role ``tool_dash/client`` in ``/srv/salt/tool_dash/parameters/roles/tool_dash/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_dash/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_dash/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_dash/parameters/dns:domain/  '/srv/salt/tool_dash/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-dash-as-json:
      test.nop:
        - name: {{ dash | json }}
...
.. code-block:: sls
    ...
    dash-config-file-file-managed:
      file.managed:
        - name: {{ dash.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='dash-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ dash.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``dash`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ dash['winner'] }}
